...
def select_face():
    args['source_img'] = filedialog.askopenfilename(title="Select a face", filetypes=[("PNG","*.png"),("JPG","*.jpg"),("JPEG","*.jpeg"),("TIFF","*.tif")])
    preview_image(args['source_img'])


def select_target():
    args['target_path'] = filedialog.askopenfilename(title="Select a target",filetypes=[("MOV","*.mov"),("MP4","*.mp4")])
